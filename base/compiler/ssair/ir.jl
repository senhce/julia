    ...
    function IncrementalCompact(code::IRCode, allow_cfg_transforms::Bool=true)
        ...
        if allow_cfg_transforms
            ...
            for i = 1:length(bb_rename)
                ...
                map!(i->bb_rename[i], blocks[i].preds, blocks[i].preds)
                # Dead blocks get removed from the predecessor list
                filter!(x->x !== 0, blocks[i].preds)
                map!(i->bb_rename[i], blocks[i].succs, blocks[i].succs)
            end
...
function kill_edge!(bbs::Vector{BasicBlock}, from, to)
    ...
    if length(preds) == 0
        for succ in copy(blocks[to].succs)
    ...
    if length(preds) == 0
        ...
        if to < active_bb
            ...
            while idx < length(compact.result) &&
                    (stmt = compact.result[idx]; isa(stmt, PhiNode))
...
function iterate(it::CompactPeekIterator, (idx, aidx, bidx)=(it.start_idx,it.compact.new_nodes_idx,1))
    # TODO: Take advantage of the fact that these arrays are sorted
    compact = it.compact
    if compact.new_nodes_idx <= length(compact.perm)
        r = aidx:length(compact.perm)
        entry_idx = findfirst(eidx->entry_at_idx(compact.ir.new_nodes[compact.perm[eidx]], idx), r)
        if entry_idx != nothing
            entry_idx = r[entry_idx]
            entry = compact.ir.new_nodes[compact.perm[entry_idx]]
            return (entry.node, (idx, entry_idx+1, bidx))
        end
    end
    if !isempty(compact.pending_perm)
        r = bidx:length(compact.pending_perm)
        entry_idx = findfirst(eidx->entry_at_idx(compact.pending_nodes[compact.pending_perm[eidx]], idx), r)
        if entry_idx != nothing
            entry_idx = r[entry_idx]
            entry = compact.pending_nodes[compact.compact.pending_perm[entry_idx]]
            return (entry.node, (idx, aidx, entry_idx+1))
        end
    end
